...
<head>
    <meta charset="utf-8">
    <title>JSDoc: Source: methods/transform.js</title>
...
<div id="main">
    ...
    <h1 class="page-title">Source: methods/transform.js</h1>
    ...
    <section>
        <article>
            <pre class="prettyprint source linenums"><code>const convertTransformStringToMatrix = (transformString) => {
  const transform = new DOMMatrix(transformString);
  return transform;
}

/**
 * Get the X and Y translate values of an element
...
const getTranslate = (element) => {
  ...
  const {e,f} = transform == 'none' ? {e:0,f:0} : new DOMMatrix(transform);
  return {x:e, y:f};
  
}

export { getTranslate, convertTransformStringToMatrix }
